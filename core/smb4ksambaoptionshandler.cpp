...
const QString Smb4KSambaOptionsHandler::mountOptions( Smb4KShare *share )
{
  Q_ASSERT( share );

  Smb4KSambaOptionsInfo *options_info  = findItem( share, true );
  ...
  if ( Smb4KSettings::remoteFileSystemPort() == 139 || (options_info && options_info->port() == 139) )
  {
    ...
    {
      args += QString( ",netbiosname=%1" ).arg( KShell::quoteArg( Smb4KSettings::netBIOSName() ) );
    }
    else
    {
      if ( !global_options["netbios name"].isEmpty() )
      {
        args += QString( ",netbiosname=%1" ).arg( KShell::quoteArg( global_options["netbios name"] ) );
      }
      else
      {
        // Do nothing
    ...
    args += ",servern="+KShell::quoteArg( share->hostName() );
  }
  else
  {
    // Do nothing
  ...
  // UID
  args += QString( ",uid=%1" ).arg( options_info ?
                                    options_info->uid() :
                                    (uid_t)Smb4KSettings::userID().toInt() );

  // GID
  args += QString( ",gid=%1" ).arg( options_info ?
                                    options_info->gid() :
                                    (gid_t)Smb4KSettings::groupID().toInt() );
  ...
  switch ( Smb4KSettings::clientCharset() )
  {
    case Smb4KSettings::EnumClientCharset::default_charset:
    {
      ...
      {
        args += QString( ",iocharset=%1" ).arg( global_options["unix charset"].toLower() );
      }
      else
      {
        // Do nothing
    ...
    default:
    {
      ...
      args += QString( ",iocharset=%1" ).arg( Smb4KSettings::self()->clientCharsetItem()->label() );
      break;
    }
  }

  // Port
  args += QString( ",port=%1" ).arg( (options_info && options_info->port() != -1) ?
                                     options_info->port() :
  ...
  // File mask
  args += !Smb4KSettings::fileMask().isEmpty() ?
          QString( ",file_mode=%1" ).arg( Smb4KSettings::fileMask() ) :
          "";

  // Directory mask
  args += !Smb4KSettings::directoryMask().isEmpty() ?
          QString( ",dir_mode=%1" ).arg( Smb4KSettings::directoryMask() ) :
          "";

  // Permission checks
  args += Smb4KSettings::permissionChecks() ?
          ",perm" :
          ",noperm";

  // Client controls IDs
  args += Smb4KSettings::clientControlsIDs() ?
          ",setuids" :
          ",nosetuids";

  // Server inode numbers
  args += Smb4KSettings::serverInodeNumbers() ?
          ",serverino" :
          ",noserverino";

  // Inode data caching
  args += Smb4KSettings::noInodeDataCaching() ?
          ",directio" :
          "";

  // Translate reserved characters
  args += Smb4KSettings::translateReservedChars() ?
          ",mapchars" :
          ",nomapchars";

  // Locking
  args += Smb4KSettings::noLocking() ?
          ",nolock" :
          "";

  // Global custom options provided by the user
  args += !Smb4KSettings::customCIFSOptions().isEmpty() ?
...
#else
  // The UID
  args += QString( " -u %1" ).arg( options_info ?
                                   options_info->uid() :
                                   (uid_t)Smb4KSettings::userID().toInt() );

  // The GID
  args += QString( " -g %1" ).arg( options_info ?
                                   options_info->gid() :
                                   (uid_t)Smb4KSettings::groupID().toInt() );
  ...
  {
    args += QString( " -E %1:%2" ).arg( charset, codepage );
  }
  else
  {
    // Do nothing
  ...
  {
    args += QString( " -f %1" ).arg( Smb4KSettings::fileMask() );
  }
  else
  {
    // Do nothing
  ...
  {
    args += QString( " -d %1" ).arg( Smb4KSettings::directoryMask() );
  }
  else
  {
    // Do nothing
